...
def deploy_workload(
    ...
    *,
    ...
    paths = resolve_paths(workload_stack="21-workload-mcp")
    ...
    state_key = workload_state_key(env, "mcp")
    update_tfvars(
        tfvars,
        {
...
def _log_app_endpoints(workload_path: Path) -> None:
    ...
    if app_fqdn:
        logging.info("MCP app: %s", f"{_normalize(app_fqdn)}/")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build, push, and deploy the 21-workload-mcp stack."
    )
    parser.add_argument(
        "env",
