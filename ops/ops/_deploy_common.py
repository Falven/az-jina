...
def resolve_paths(workload_stack: str = "20-workload") -> Paths:
    root = repo_root()
    stacks = root / "infra" / "terraform" / "stacks"
    return Paths(
        root=root,
        bootstrap=stacks / "00-bootstrap",
        workload=stacks / workload_stack,
    )


def workload_state_key(env: str, stack_key: str) -> str:
    if env.strip() == "":
        raise ValueError("env must not be empty")
    if stack_key.strip() == "":
        raise ValueError("stack_key must not be empty")
    return f"{env}/{stack_key}/terraform.tfstate"
