...
def build_and_push(
    *,
    ...
    paths = resolve_paths(workload_stack=workload_stack)
    ctx = azure_context()
    tfvars = (
        tfvars_path
    ...
    image_repo = os.environ.get(image_repo_env, default_image_repo)
...
def build_cli(
    *,
    ...
    parser.add_argument(
        "--tfvars",
        type=Path,
        help="Optional path to tfvars to update (defaults to stacks/<workload_stack>/<env>.tfvars)",
    )
    ...
    build_and_push(
        ...
        workload_stack=workload_stack,
        image_repo_env=image_repo_env,
        default_image_repo=default_image_repo,
        registry_login_env=registry_login_env,
...
def _registry_login_server(
    ...
    value = (
        override
        or os.environ.get(registry_login_env)
    ...
    if not value or "<" in value or value.strip() == "":
        raise RuntimeError(
            ...
            f"or export {registry_login_env}."
        )
    return value
